<template>
  <div class="d-block">
    <div
      v-if="error.length > 0 || errMessage"
      class="alert alert-custom alert-light-danger fade show mb-5 py-2 px-4 w-50"
      role="alert"
    >
      ...
      <div class="alert-text">
        ...
        <div class="error-wrapper" v-if="errMessage">
          <span class="d-block font-size-sm"
            >{{ $t("hint.failed_upload") }}, {{ errMessage }}</span
          >
        </div>
      </div>
    </div>
      ...
      <div class="input-desc">
        <div>
          ...
          <span class=" font-weight-bold font-size-sm"
            >{{ $t("hint.max_size") }}
            {{ attributes.filter(x => x.limit)[0].limit }} Mb,
            {{ $t("hint.max_file") }}
            {{ attributes.filter(x => x.max)[0].max }} File
          </span>
        </div>
        <div class="mt-3">
          <div class="uploaded-wrapper">
            ...
            <template v-for="(f, i) in value">
              <div v-if="f[columnId ? columnId : 'file']" :key="i" class="img-multi-wrapper">
                <img
                  v-img-fb="f[columnId ? columnId : 'file'].url || './static/img/default/camera.svg'"
                  alt=""
                />
                <button
                  type="button"
...
export default {
  ...
  props: {
    ...
    attr: {
      ...
      required: false,
      default: () => {
        return [{ accept: "image/*" }, { limit: 1 }, { max: 5 }];
      }
    },
    value: { type: Array, required: false },
    viewData: { type: String, required: false }
  },
  data() {
    return {
      isUploading: false,
  ...
  methods: {
    handleDelete(index) {
      ...
      else this.$set(this.value[index], "file", null);
      // this.$set(this.tempFile[index], "url", null);
    },
    handleFile(e) {
      if (this.value.length < this.attributes.filter(x => x.max)[0].max) {
        if (
          (e.target.files[0].size / 1000000).toFixed(2) <
          this.attributes.filter(x => x.limit)[0].limit
        ) {
