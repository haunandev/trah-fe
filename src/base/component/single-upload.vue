<template>
  <div class="img-form-wrapper">
    <div class="input-img">
      <label>
        <a>
          <img
            v-if="uploadURL"
            v-img-fb="uploadURL.url || './static/img/default/camera.svg'"
            alt=""
          />
    ...
    <div v-if="!hideDesc" class="input-desc">
      ...
      <div
        class="mt-3"
        v-if="uploadDetail && value && !errMessage && error.length === 0"
      >
        <span class="d-block text-muted font-size-sm"
          >{{ $t("global.file_name") }}:
          <a :href="value" target="_blank">{{ uploadDetail.name }}</a></span
        >
        <span class="d-block text-muted font-size-sm"
          >{{ $t("global.size") }}:
          {{ (uploadDetail.size / 1000000).toFixed(2) }} Mb</span
        >
        <span class="d-block text-muted font-size-sm"
          >{{ $t("global.type") }}: {{ uploadDetail.type }}</span
        >
        <!-- <span
          class="badge badge-danger badge-sm mt-3 pointer"
      ...
      <div class="mt-3" v-else-if="errMessage">
        <span class="d-block text-danger font-size-sm">{{
          $t("hint.failed_upload")
        }}</span>
