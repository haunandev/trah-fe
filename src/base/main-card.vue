<template>
  <div class="card-wrapper">
      ...
      <div class="card-header border-0 py-0">
        <div class="card-title flex-fill mr-0">
          <div class="input-icon w-100">
            <input
              v-model.lazy.trim="paramAPI.search"
              autocomplete="off"
              name="search"
              type="text"
    ...
    <masonry
      ...
      :cols="{ default: 3, 1024: 2, 768: 2, 425: 1 }"
      :gutter="{ default: '.75rem', 700: '15px' }"
    >
      <div
        class="card card-custom mb-3"
        v-for="(b, i) in mainData"
        :key="i + '-item-card'"
      >
    ...
    <div
      v-if="mainData.length > 0"
      class="d-flex flex-column align-items-center justify-content-center text-center my-6"
    >
      <b-spinner
        v-if="isLoading"
        variant="primary"
      ...
      ></b-spinner>
      <button
        v-if="!isLoading && !(paramAPI.page == maxPage)"
        @click="goPage('next')"
      ...
      <span
        ...
        @click="goPage('next')"
        class="text-muted"
        >Semua data sudah ditampilkan</span
      >
    </div>
...
export default {
  ...
  methods: {
    ...
    getData(param, reload = false) {
      ...
      this.$_api.list(this.API, param).then(
        e => {
          e.total = e.record;
          let data = e.data;
          this.total = e.total;
      ...
      this.$_alert
        ...
        .then(result => {
          if (result.isConfirmed) {
            this.$_api
              .delete(this.endpoint.delete, { data: data })
              .then(res => {
