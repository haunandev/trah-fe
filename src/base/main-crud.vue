  ...
  <div :class="[slave ? 'container-fluid p-0 mt-5' : classes || 'container']">
    <template v-if="!isLoading">
      ...
      >
        <slot name="create-form">
          <tas-base-form
            :slave="slave"
            @back="
              config.redirectAfterForm
                ? onSuccessForm('create', null)
                : getEvent('list')
            "
            @success="onSuccessForm('create', $event)"
            :key="config.title + '-form-create'"
            :form="'create'"
            :API="{ setter: configAPI.create, getter: configAPI.read }"
            :title="config.title"
          ...
          >
            ...
            <template v-slot:custom-fields="rowData">
      ...
      >
        <slot
          name="update-form"
          v-bind:data="{ id: activeID, data: activeData }"
        >
          <tas-base-form
            :slave="slave"
            :custom-getter="customGetter"
            @back="
              config.redirectAfterForm
                ? onSuccessForm('update', null)
                : getEvent('list')
            "
            @success="onSuccessForm('update', $event)"
            :key="config.title + '-form-update'"
            :id="activeID"
            :form="'update'"
            :API="{ setter: configAPI.create, getter: configAPI.read }"
            :title="config.title"
          ...
          >
            ...
            <template v-slot:custom-fields="rowData">
              <slot
                name="custom-fields"
                v-bind:rowData="rowData.rowData"
              ></slot>
            </template>
          </tas-base-form>
        </slot>
      ...
      >
        ...
        <tas-base-detail
          :isBodySlot="!!$scopedSlots['detail-body']"
          :isCustomAPI="config.custom_api ? !!config.custom_api.read : false"
          @zoom="getEvent('zoom', $event)"
          @back="getEvent('list')"
          :key="config.title + '-detail'"
          :id="activeID"
          :slave="config.slave"
        ...
        >
          <template v-slot:detail-card-body="rowData">
            <slot
              name="detail-card-body"
              v-bind:rowData="{ ...rowData.rowData, ...activeData }"
            ></slot>
          </template>
      ...
      <div id="list" ref="list" class="animated" v-if="activeView === 'list'">
        ...
        <slot name="list-body">
          <tas-base-table
            ...
            v-if="!listCard"
            @zoom="getEvent('zoom', $event)"
            @add_new="getEvent('create')"
            @filter="getEvent('filter')"
            @export="getEvent('export')"
            @import="getEvent('import')"
            @detail="getEvent('detail', $event)"
            @update="getEvent('update', $event)"
            @delete="getEvent('delete', $event)"
            :key="config.title + '-list'"
            :slave="slave"
            :permission="permission"
            :title="config.title"
            :API="configAPI.list"
            :isCustomAPI="config.custom_api ? !!config.custom_api.list : false"
            :endpoint="configAPI"
            :fields="fields.list"
            :export="config.export"
          ...
          >
            ...
            <template v-if="!advanceFilter" v-slot:inline-filter>
              <slot name="list-filter">
                <tas-base-form
                  class="animated"
                  btnSaveLabel="Terapkan Filter"
                  v-show="filterView"
                  @back="getEvent('list')"
                  @success="onSuccessForm('filter', $event)"
                  :key="config.title + '-inline-form-filter'"
                  simple
                  :form="'filter'"
                  :API="{ setter: configAPI.create, getter: configAPI.read }"
                  :title="config.title"
            ...
            <!-- table data header -->
            <template v-slot:list-header-table>
              <slot name="list-header-table"></slot>
            </template>
            <!-- table data -->
      ...
      <div id="list" ref="list" class="animated" v-if="!activeView">
        <not-found></not-found>
      </div>
    </template>
    <template v-else>
    ...
    <b-sidebar
      v-if="!isLoading && advanceFilter"
      @hidden="filterView = false"
      :visible="filterView"
      id="filter-component"
      backdrop
  ...
  computed: {
    slave() {
      if (this.$store.state.activeSlave && this.$parent.$parent.title) {
        if (this.$store.state.activeSlave.master === this.$parent.$parent.title)
          return true;
        else return false;
      } else return false;
    }
    ...
    config: {
      ...
      handler(to, from) {
        if (this.slave && this.$store.state.activeSlave.rules) {
          let rule = JSON.parse(
            JSON.stringify(this.$store.state.activeSlave.rules)
          );
    ...
    activeView: {
      ...
      handler(to) {
        if (!this.slave) {
          if (
            to &&
            this.$route.query.view !== to &&
            ["create", "list", "detail", "update"].includes(
    ...
    $route: {
      ...
      handler(to) {
        if (!this.slave) {
          ...
          this.activeID = to.query.id ? Number(to.query.id) : "";
        } else {
  ...
  methods: {
    ...
    moduleChange(config) {
      ...
      if (config.custom_api) {
        this.configAPI = {
          ...
          create: config.custom_api.create
            ? config.custom_api.create
            : config.setter,
          read: config.custom_api.read ? config.custom_api.read : config.getter,
          update: config.custom_api.update
            ? config.custom_api.update
            : config.setter,
          delete: config.custom_api.delete
            ? config.custom_api.delete
            : config.setter
        };
        ...
        this.configAPI = {
          list:
            this.slave &&
            this.$store.state.activeSlave.rules &&
            this.$store.state.activeSlave.rules.getter
              ? this.$store.state.activeSlave.rules.getter
              : config.getter,
          create:
            this.slave &&
            this.$store.state.activeSlave.rules &&
            this.$store.state.activeSlave.rules.getter
              ? this.$store.state.activeSlave.rules.setter
              : config.setter,
          read:
            this.slave &&
            this.$store.state.activeSlave.rules &&
      ...
      if (config.model_api) {
        this.$_api.get("/gen-model/" + config.model_api).then(
          res => {
            this.generateFieldAPI(res).then(result => {
              this.buildField(config.fields, result, config.pk_field).then(
                result => {
                  this.fields = result;
    ...
    async generateFieldAPI(res) {
      ...
      res.fieldList.forEach(el => {
        let _field = {
          ...
          placeholder: null,
          methods: {
            list: res.fieldList.find(x => x === el)
              ? this.defineTypeAPI(res.fieldType[el], res, el, "list")
              : false,
            detail: res.fieldView.find(x => x === el)
              ? this.defineTypeAPI(res.fieldType[el], res, el, "detail")
              : false,
            create: res.fieldAdd.find(x => x === el)
              ? this.defineTypeAPI(res.fieldType[el], res, el, "create")
              : false,
            update: res.fieldEdit.find(x => x === el)
              ? this.defineTypeAPI(res.fieldType[el], res, el, "update")
              : false,
    ...
    defineTypeAPI(item, data, pointer, from) {
      ...
      if (_tmp !== "unset" && data.fieldValidation[pointer]) {
        ...
        _tmp.validation = data.fieldValidation[pointer]
          .split("|")
    ...
    async buildField(local, server, pk = null) {
      ...
      let data =
        this.slave &&
        this.$store.state.activeSlave.rules &&
        this.$store.state.activeSlave.rules.fields
          ? this.getNewProps(_data, this.$store.state.activeSlave.rules.fields)
          : _data;

      let sortedData = pk ? this.swapPrimaryField(data, pk) : data;
      await sortedData.forEach(async item => {
        let def = {
          id: item.id,
        ...
        if (item.methods) {
          if (item.methods.list) {
            if (typeof item.methods.list === "object") {
              list = {
                show:
                  item.methods.list.show === undefined
                    ? true
                    : item.methods.list.show,
                order:
                  item.methods.list.order === undefined
                    ? true
                    : item.methods.list.order,
                view_data: item.methods.list.view_data || def.data,
                permission:
                  item.methods.list.permission || "template-allow-all",
          ...
          if (item.methods.detail) {
            if (typeof item.methods.detail === "object") {
              detail = {
                show:
                  item.methods.detail.show === undefined
                    ? true
                    : item.methods.detail.show,
                order:
                  item.methods.detail.order === undefined
                    ? true
                    : item.methods.detail.order,
                permission:
                  item.methods.detail.permission || "template-allow-all",
          ...
          if (item.methods.create) {
            if (typeof item.methods.create === "object") {
              create = {
                show:
                  item.methods.create.show === undefined
                    ? true
                    : item.methods.create.show,
                view_data: item.methods.create.view_data || def.data,
                permission:
                  item.methods.create.permission || "template-allow-all",
                ...
                validation: Array.isArray(item.methods.create.validation)
                  ? item.methods.create.validation
                  : [],
                event: item.methods.create.event,
                option: item.methods.create.option,
                tooltip: item.methods.create.tooltip || null,
              };
            } else {
              create = {
                show: true,
          ...
          if (item.methods.update) {
            if (typeof item.methods.update === "object") {
              update = {
                show:
                  item.methods.update.show === undefined
                    ? true
                    : item.methods.update.show,
                view_data: item.methods.update.view_data || def.data,
                permission:
                  item.methods.update.permission || "template-allow-all",
                ...
                validation: Array.isArray(item.methods.update.validation)
                  ? item.methods.update.validation
                  : [],
                event: item.methods.update.event,
                option: item.methods.update.option,
                tooltip: item.methods.update.tooltip || null,
              };
            } else {
              update = {
                show: true,
          ...
          if (item.methods.filter) {
            if (typeof item.methods.filter === "object") {
              filter = {
                show:
                  item.methods.filter.show === undefined
                    ? true
                    : item.methods.filter.show,
                view_data: item.methods.filter.view_data || def.data,
                permission:
                  item.methods.filter.permission || "template-allow-all",
                ...
                validation: Array.isArray(item.methods.filter.validation)
                  ? item.methods.filter.validation
                  : [],
    ...
    onSuccessForm(from, e) {
      if (from === "update") {
        if (this.config.redirectAfterForm) {
          this.$router.push(this.config.redirectAfterForm).catch(err => {
            return err;
          });
        } else {
          ...
          this.$_alert.success(
            null,
            "Edit data " + this.config.title + " berhasil"
          );
        }
      } else if (from === "create") {
        if (this.config.redirectAfterForm) {
          ...
          this.$_alert.success(
            null,
            "Data " + this.config.title + " berhasil ditambahkan"
          );
        }
      } else if (from === "filter") {
        this.filters = {
          ...this.$_sys.cleansingObj(e),
