<template>
  <div>
    ...
    <table class="table no-border mb-0">
      <tr>
        <td class="nowrap-table pl-0">NIP</td>
        <th class="pr-0">{{ profile.nip || '-' }}</th>
      </tr>
      <tr>
        <td class="nowrap-table pl-0">Jabatan</td>
        <th class="pr-0">{{ profile.role_name }}</th>
      </tr>
      <tr>
      ...
      <template v-if="formProfile">
        <ValidationObserver v-slot="{ handleSubmit  }" ref="formProfileObserver">
          <form autocomplete="off" @submit.prevent="handleSubmit(changeProfile)">
            <div class="row">
              <div class="col-auto">
                <single-upload hide-desc v-model.lazy="formProfile.img_photo_user" :attr="[{ accept:'.jpg, .pdf, .jpeg, .gif, .png' }, { limit: 2 }]" :error="[]" />
              </div>
              <div class="col">
            ...
            <div class="row">
              <div class="col-12">
                <span class="d-block card-label font-weight-bold text-dark h6 mb-4">Informasi Kontak</span>
              </div>
              <div class="col-6">
              ...
              <div class="col-6">
                <s-input
                  v-if="!formProfileDisabled.role_id"
                  v-model="formProfile.role_id"
                  :item="{
                    label: 'Jabatan',
                    type: 'lookup-radio',
                    validation: ['required'],
                ...
                />
                <s-input v-else v-model="formProfile.role_name" :item="{ label:'Jabatan', type:'text', disabled: true, validation:[], placeholder: '', api:'' }" />
              </div>
...
export default {
  ...
  methods: {
    ...
    showModalProfile () {
      ...
      if (!permission.includes('update-roles')) this.formProfileDisabled.role_id = true
