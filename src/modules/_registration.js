...
requireModules.keys().forEach(folder => {
  if (modulesSorter.indexOf(folder.split("/")[1]) > -1) {
    moduleList[modulesSorter.indexOf(folder.split("/")[1])] = folder;
  ...
  for (var j = 0; j < finalModule.length; j++) {
    ...
    for (var i = 0; i < appModules.routes.length; i++) {
      ...
      if (r.component) {
        ...
        var config = data.default.data().config;
